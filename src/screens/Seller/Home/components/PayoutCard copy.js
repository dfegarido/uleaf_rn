    ...
    <View style={styles.card}>
      ...
      <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
        <View>
          ...
          <Text style={[globalStyles.textSMGreyDark]}>WW{item.workWeek}</Text>
          <Text style={styles.label}>Work week</Text>
        </View>
        <View style={styles.headerRight}>
          <Text style={[globalStyles.textLGGreyDark, globalStyles.textBold]}>
            {item.totalReceivableAmountCurrencySymbol}
            {Number(item.totalReceivableAmountLocal).toLocaleString()}
          </Text>
          <Text style={styles.label}>
            Total amount ({item.totalReceivableAmountCurrency})
          </Text>
        </View>
      </View>
    </View>
