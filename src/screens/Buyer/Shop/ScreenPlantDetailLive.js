...
import { getPlantDetailApiLive } from '../../../components/Api/getPlantDetailApi';
import { retryAsync } from '../../../utils/utils';

const ScreenPlantDetailLiveModal = ({navigation, plantCode, isVisible, onClose}) => {
  const {user} = useAuth();
  ...
  const [modalAction, setModalAction] = useState('buy-now'); // 'add-to-cart' or 'buy-now'
  ...
  const loadPlantDetails = async () => {
    try {
      ...
      const res = await retryAsync(() => getPlantDetailApiLive(plantCode), 3, 1000);
...
export default ScreenPlantDetailLiveModal;
