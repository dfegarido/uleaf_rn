...
const BuyXGetY = () => {
  ...
  const [buyQuantity, setDiscountPercent] = useState('');
  const [getQuantity, setMaxDiscount] = useState('');
  ...
  useEffect(() => {
    const loadGenus = async () => {
      ...
      try {
        ...
        const res = await getGenusApi();
        if (res?.success && Array.isArray(res.data)) {
          setGenusOptions(res.data.map(item => item.name).filter(Boolean));
        } else {
          setGenusOptions([]);
        }
      } catch (e) {
  ...
  useEffect(() => {
    const loadGardens = async () => {
      ...
      try {
        ...
        const primary = res?.garden || res?.data?.garden || res?.gardenList || res?.gardens || res?.data?.gardens || [];
        let names = Array.isArray(primary)
          ? primary.map(g => (typeof g === 'string' ? g : (g.name || g.garden || String(g)))).filter(Boolean)
          : [];
        let uniqueSorted = Array.from(new Set(names)).sort((a, b) => a.localeCompare(b));

        // Fallback: derive gardens from listings if too few
        if (uniqueSorted.length <= 1) {
          const listingsResp = await getAdminListingsApi({ page: 1, limit: 500 });
          const listings = listingsResp?.data?.listings || [];
          const fromListings = listings
            .map(l => l?.gardenOrCompanyName || l?.garden || l?.gardenName || l?.supplierCompany || l?.sellerCompany)
            .filter(Boolean)
            .map(v => String(v));
          uniqueSorted = Array.from(new Set([...uniqueSorted, ...fromListings])).sort((a, b) => a.localeCompare(b));
        }

        // Second fallback: derive from admin orders across statuses and pages
        if (uniqueSorted.length <= 1) {
          try {
            const statuses = ['readyToFly', 'completed', 'wildgone'];
            const aggregated = new Set(uniqueSorted);
            for (let sIdx = 0; sIdx < statuses.length; sIdx++) {
              const status = statuses[sIdx];
              // Fetch up to 5 pages x 200 = 1000 orders per status
              for (let page = 1; page <= 5; page++) {
                const ordersResp = await getAdminOrdersApi({ limit: 200, page, sort: 'latest', status });
                const orders = ordersResp?.orders || ordersResp?.data?.orders || [];
                if (!Array.isArray(orders) || orders.length === 0) break;
                for (let i = 0; i < orders.length; i++) {
                  const o = orders[i];
                  const name = o?.gardenOrCompanyName || o?.garden || o?.gardenName;
                  if (name) aggregated.add(String(name));
                }
                // Stop if fewer than limit returned (likely last page)
                if (orders.length < 200) break;
              }
            }
            uniqueSorted = Array.from(aggregated).sort((a, b) => a.localeCompare(b));
          } catch (e) {
            // ignore
          }
        }

        setGardenOptions(uniqueSorted);
      } catch (e) {
        setGardenOptions([]);
      } finally {
  ...
  useEffect(() => {
    const loadBuyers = async () => {
      ...
      try {
        ...
        // If API didn't filter by role server-side, enforce buy-only here
    ...
    buyerSearchDebounceRef.current = setTimeout(async () => {
      try {
        if (q.length >= 2) {
          ...
          const res = await searchBuyersApi({ query: q, limit: 4, offset: 0 });
          if (!res?.success) {
  ...
  return (
    <SafeAreaView style={{flex: 1, backgroundColor: '#FFFFFF'}}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Buy X Get Y</Text>
      </View>
      ...
      <ScrollView style={{flex: 1}} contentContainerStyle={{paddingBottom: 24}} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.sectionPad}>
          <View style={styles.fieldGroup}>
            <Text style={styles.label}>Buy (quantity)<Text style={styles.reqAsterisk}>*</Text></Text>
            <View style={{flexDirection: 'row'}}>
              ...
              <View style={[styles.suffixBox]}> 
                <Text style={styles.suffixText}>{// isFixed ? '' : ''}</Text>
              </View>
            </View>
          </View>
        </View>
        ...
        <View style={styles.sectionPad}>
          <View style={styles.fieldGroup}>
            <Text style={styles.label}>Get (quantity)<Text style={styles.reqAsterisk}>*</Text></Text>
            <View style={{flexDirection: 'row'}}>
        ...
        {!!selectedGardens.length && appliesText === 'Specific garden' && (
          <View style={styles.appliesListWrap}>
            {selectedGardens.map((g, idx) => (
              <View key={`${g}-${idx}`} style={styles.appliesCard}>
                <Text style={styles.appliesCardText}>{g}</Text>
                <TouchableOpacity onPress={() => setSelectedGardens(prev => prev.filter(item => item !== g))}>
                  <Svg width={24} height={24} viewBox="0 0 24 24" fill="none">
                    <Path fillRule="evenodd" clipRule="evenodd" d="M4.71967 4.71967C5.01256 4.42678 5.48744 4.42678 5.78033 4.71967L12 10.9393L18.2197 4.71967C18.5126 4.42678 18.9874 4.42678 19.2803 4.71967C19.5732 5.01256 19.5732 5.48744 19.2803 5.78033L13.0607 12L19.2803 18.2197C19.5732 18.5126 19.5732 18.9874 19.2803 19.2803C18.9874 19.5732 18.5126 19.5732 18.2197 19.2803L12 13.0607L5.78033 19.2803C5.48744 19.5732 5.01256 19.5732 4.71967 19.2803C4.42678 18.9874 4.42678 18.5126 4.71967 18.2197L10.9393 12L4.71967 5.78033C4.42678 5.48744 4.42678 5.01256 4.71967 4.71967Z" fill="#7F8D91"/>
                  </Svg>
                </TouchableOpacity>
        ...
        {!!selectedListings.length && appliesText === 'Specific listing' && (
          <View style={styles.appliesListWrapListings}>
            {selectedListings.map((listing, idx) => {
              ...
              const buyQuantity = listing.buyQuantity || v0.buyQuantity;
                      ...
                      <View style={styles.badgeRowFull}>
                        ...
                        {buyQuantity && (
                          <View style={styles.discountBadgeFull}>
                            <Text style={styles.buyQuantityTextFull}>{buyQuantity}%</Text>
              ...
              {!!selectedBuyers.length && (
                <View style={[styles.appliesListWrap, {marginTop: 8, marginHorizontal: -24}]}> 
                  {selectedBuyers.map((b, idx) => (
                    <View key={`${b.id}-${idx}`} style={styles.appliesCard}>
        ...
        <View style={styles.boxSpecs}> 
          ...
          <View style={styles.boxDetails}>
            <View style={styles.boxRow}><Text style={styles.boxLabel}>Code</Text><Text style={styles.boxValue}>{code || 'SUMMER25'}</Text></View>
              ...
              <View style={[styles.typeSheetContainer, {position: 'absolute', left: (SCREEN.width - 340) / 2, top: typeSheetTop}] }>
                <View style={styles.typeRowWrapper}>
                  <TouchableOpacity style={styles.typeRowLeft} onPress={() => { 
                    ...
                    navigation.replace('AdminDiscountBuyXGetY');
                  }}>
                    <Svg width={24} height={24} viewBox="0 0 24 24" fill="none">
                ...
                <View style={styles.typeRowWrapper}>
                  <TouchableOpacity style={styles.typeRowLeft} onPress={() => { 
                    ...
                    setShowTypeSheet(false); 
                    navigation.replace('AdminDiscountBuyXGetY', { mode: 'fixed' });
                  }}>
                    <Svg width={24} height={24} viewBox="0 0 24 24" fill="none">
          ...
          <View style={styles.buyerOverlay}>
            <TouchableWithoutFeedback>
              <View style={{ width: '100%', height: 582, backgroundColor: '#FFFFFF', borderTopLeftRadius: 24, borderTopRightRadius: 24, position: 'absolute', bottom: 0 }}>
                ...
                <View style={styles.genusActions}>
          ...
          <View style={styles.buyerOverlay}>
            <TouchableWithoutFeedback>
              <View style={{ width: '100%', height: 354, backgroundColor: '#FFFFFF', borderTopLeftRadius: 24, borderTopRightRadius: 24, position: 'absolute', bottom: 0, transform: timeKeyboardOffset ? [{ translateY: -timeKeyboardOffset }] : [] }}>
                ...
                <View style={[styles.genusActions, {paddingTop: 24}] }>
                ...
                <View style={styles.buyerSheetContainer}>
                  ...
                  >
                    ...
                    {!gardenLoading && (gardenOptions || [])
                      .filter(name => (name || '').toLowerCase().includes(gardenSearch.toLowerCase()))
                      .map((name, idx) => (
                      <View key={`${name}-${idx}`}>
                        <View style={{flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', height: 39}}>
                          <View style={{flexDirection: 'row', alignItems: 'center'}}>
                            <View style={{width: 40, height: 40, borderRadius: 20, backgroundColor: '#C0DAC2', borderWidth: 1, borderColor: '#539461'}} />
                            <Text style={[styles.typeRowText, {fontWeight: '700', color: '#202325'}]}>{name}</Text>
                          </View>
                          <TouchableOpacity onPress={() => {
                            setSelectedGardens(prev => prev.includes(name) ? prev.filter(g => g !== name) : [...prev, name]);
                          }}>
                            <View style={[styles.checkboxSquare, selectedGardens.includes(name) ? styles.checkboxSquareSelected : styles.checkboxSquareDefault]}>
                              {selectedGardens.includes(name) && (
                                <Svg width={16} height={16} viewBox="0 0 24 24" fill="none">
                                  <Path d="M5 13L9 17L19 7" stroke="#FFFFFF" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                                </Svg>
                              )}
          ...
          <View style={styles.buyerOverlay}>
            <TouchableWithoutFeedback>
              <View style={{width: '100%'}}>
                <View style={[
                  styles.buyerSheetContainer,
                  buyerKeyboardOffset ? { transform: [{ translateY: -buyerKeyboardOffset }] } : null,
                ]}>
                <View style={styles.genusHeader}>
                ...
                >
                  ...
                  {!buyerLoading && buyerOptions.map((buyer, idx) => (
                    <View key={`${buyer.id}-${idx}`}>
                      ...
                      <View style={{flexDirection: 'row', alignItems: 'center'}}>
                        <View style={{width: 40, height: 40, borderRadius: 20, backgroundColor: '#C0DAC2', borderWidth: 1, borderColor: '#539461'}} />
                        <Text style={[styles.typeRowText, {fontWeight: '700', color: '#202325'}]}>{buyer.name}</Text>
                      </View>
                      <TouchableOpacity onPress={() => {
                        setSelectedBuyers(prev => prev.some(b => b.id === buyer.id) ? prev.filter(b => b.id !== buyer.id) : [...prev, buyer]);
                      }}>
                        <View style={[styles.checkboxSquare, selectedBuyers.some(b => b.id === buyer.id) ? styles.checkboxSquareSelected : styles.checkboxSquareDefault]}>
                          {selectedBuyers.some(b => b.id === buyer.id) && (
                            <Svg width={16} height={16} viewBox="0 0 24 24" fill="none">
                              <Path d="M5 13L9 17L19 7" stroke="#FFFFFF" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                            </Svg>
                          )}
      ...
      <Modal transparent visible={showGenusSheet} onRequestClose={() => setShowGenusSheet(false)} animationType="fade">
        <TouchableWithoutFeedback onPress={() => setShowGenusSheet(false)}>
          <View style={styles.fullscreenOverlay}>
            <TouchableWithoutFeedback>
              <View style={styles.genusSheetContainer}>
                <View style={styles.genusHeader}>
                ...
                <View style={styles.genusContent}>
                  <View style={[styles.inputRow, {marginHorizontal: 8, marginBottom: 8}]}> 
                    <TextInput
                      style={styles.input}
                  ...
                  {genusLoading && (
                    <Text style={[styles.genusHint, {marginLeft: 16}]}>Loading...</Text>
                  )}
                  {!genusLoading && genusOptions
                    .filter(label => label.toLowerCase().includes(genusSearch.toLowerCase()))
                    .map((label, idx) => (
                    <View key={`${label}-${idx}`} style={styles.genusRow}>
                      <View style={styles.genusRowLeft}>
                        <Text style={styles.typeRowText}>{label}</Text>
                      </View>
                      <View style={styles.genusRowRight}>
                        <TouchableOpacity onPress={() => {
                          setSelectedGenus(prev => prev.includes(label) ? prev.filter(i => i !== label) : [...prev, label]);
                        }}>
                          <View style={[styles.checkboxSquare, selectedGenus.includes(label) ? styles.checkboxSquareSelected : styles.checkboxSquareDefault]}>
                            {selectedGenus.includes(label) && (
                              <Svg width={16} height={16} viewBox="0 0 24 24" fill="none">
                                <Path d="M5 13L9 17L19 7" stroke="#FFFFFF" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                              </Svg>
                            )}
                  ...
                  {!genusLoading && genusOptions.filter(label => label.toLowerCase().includes(genusSearch.toLowerCase())).length === 0 && (
                    <Text style={[styles.genusHint, {marginLeft: 16}]}>No results</Text>
                  )}
                </View>
                <View style={styles.genusActions}>
...
const styles = StyleSheet.create({
  ...
  fullscreenOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.45)', justifyContent: 'center', alignItems: 'center' },
  ...
  // Genus bottom sheet
  genusSheetContainer: { width: 375, height: 554, backgroundColor: '#FFFFFF', borderTopLeftRadius: 24, borderTopRightRadius: 24, position: 'absolute', bottom: 0 },
  ...
  genusContent: { paddingHorizontal: 8, paddingVertical: 8, height: 400 },
  genusRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', height: 48, paddingLeft: 16, paddingRight: 16 },
  ...
  genusActions: { flexDirection: 'row', paddingHorizontal: 24, paddingTop: 12, paddingBottom: 0 },
