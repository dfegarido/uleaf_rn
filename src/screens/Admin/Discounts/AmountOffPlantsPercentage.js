...
const AmountOffPlantsPercentage = () => {
  ...
  const initialBuyerLoadCompleteRef = React.useRef(false);
    ...
    const loadGenus = async () => {
      ...
      try {
        ...
        if (!net.isConnected || !net.isInternetReachable) {
          console.warn('⚠️ No network connection, cannot load genus data');
          setGenusLoading(false);
          return;
        }
        console.log('🌿 Loading genus data...');
        ...
        if (res?.success && Array.isArray(res.data)) {
          ...
          console.log('🌿 Processed genus list:', {
            total: genusList.length,
            firstFew: genusList.slice(0, 5)
          });
          setGenusOptions(genusList.sort((a, b) => a.localeCompare(b)));
        } else {
          console.error('❌ Invalid genus response format:', res);
          setGenusOptions([]);
        }
      } catch (e) {
        console.error('❌ Failed to load genus:', e.message || e);
        setGenusOptions([]);
      } finally {
        setGenusLoading(false);
      }
  ...
  useEffect(() => {
    const loadBuyers = async () => {
      ...
      try {
        ...
        // Clear search without triggering the search effect (use a ref or check)
          ...
          <TouchableOpacity 
            ...
            onPress={async () => {
              ...
              try {
                ...
                console.log('Create Amount Off Plants Discount:', discountData);
              ...
              <View style={styles.genusSheetWrapper}>
                ...
                <View style={styles.genusHeader}>
                  ...
                  {!genusLoading && (() => {
                    ...
                    console.log('🌿 Rendering genus list:', {
                      totalOptions: genusOptions.length,
                      searchTerm: genusSearch,
