  ...
  useEffect(() => {
    const loadGenus = async () => {
      ...
      try {
        ...
        console.log('📊 getAllPlantGenusApi response:', {
          success: res?.success,
          hasData: !!res?.data,
          dataType: Array.isArray(res?.data) ? 'array' : typeof res?.data,
          dataLength: Array.isArray(res?.data) ? res.data.length : 'not array',
          sample: Array.isArray(res?.data) ? res.data.slice(0, 5) : res?.data
        });
        ...
        if (res?.success && Array.isArray(res.data)) {
          ...
          console.log('📊 Processed genus list length:', genusList.length);
          console.log('📊 First 10 genus items:', genusList.slice(0, 10));
      ...
      <ScrollView style={{flex: 1}} contentContainerStyle={{paddingBottom: 24}} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.appliesListWrap}>
            {selectedGardens.map((garden, idx) => (
              <View key={`${garden.name}-${idx}`} style={[styles.appliesCard, selectedGardens.length > 1 && idx < selectedGardens.length - 1 && {marginBottom: 8}]}>
                ...
                <TouchableOpacity onPress={() => setSelectedGardens(prev => prev.filter(item => item.name !== garden.name))}>
                  <Svg width={24} height={24} viewBox="0 0 24 24" fill="none">
                    <Path fillRule="evenodd" clipRule="evenodd" d="M4.71967 4.71967C5.01256 4.42678 5.48744 4.42678 5.78033 4.71967L12 10.9393L18.2197 4.71967C18.5126 4.42678 18.9874 4.42678 19.2803 4.71967C19.5732 5.01256 19.5732 5.48744 19.2803 5.78033L13.0607 12L19.2803 18.2197C19.5732 18.5126 19.5732 18.9874 19.2803 19.2803C18.9874 19.5732 18.5126 19.5732 18.2197 19.2803L12 13.0607L5.78033 19.2803C5.48744 19.5732 5.01256 19.5732 4.71967 19.2803C4.42678 18.9874 4.42678 18.5126 4.71967 18.2197L10.9393 12L4.71967 5.78033C4.42678 5.48744 4.42678 5.01256 4.71967 4.71967Z" fill="#7F8D91"/>
                  </Svg>
                </TouchableOpacity>
        ...
        <View style={styles.boxSpecs}> 
          ...
          <View style={styles.boxDetails}>
            ...
            <View style={styles.boxRow}><Text style={styles.boxLabel}>Type</Text><Text style={styles.boxValue}>Buy X Get Y</Text></View>
            <View style={styles.boxRow}><Text style={styles.boxLabel}>Buy</Text><Text style={styles.boxValue}>{buyQuantity || '—'}</Text></View>
            <View style={styles.boxRow}><Text style={styles.boxLabel}>Get</Text><Text style={styles.boxValue}>{getQuantity || '—'}</Text></View>
          ...
          <View style={styles.boxList}>
            ...
            {limitTotalEnabled && maxUsesTotal ? (
              ...
              <Text style={styles.boxListItem}>{'\u2022'} No usage limit</Text>
            )}
          </View>
        </View>
          ...
          <TouchableOpacity 
            ...
            onPress={async () => {
              ...
              try {
                ...
                console.log('Create Buy X Get Y Discount:', discountData);
                      ...
                      >
                        ...
                        {!gardenLoading && (gardenOptions || [])
                          ...
                          .map((garden, idx, filteredList) => {
                            const isSelected = selectedGardens.some(g => g.name === garden.name);
                            return (
                  ...
                  <SafeAreaView style={{flex: 1}}>
                    ...
                    <View style={styles.genusContentContainer}>
                      ...
                      >
                        ...
                        {!genusLoading && (() => {
                          const filtered = genusOptions.filter(label => label.toLowerCase().includes(genusSearch.toLowerCase()));
                          console.log('📊 Rendering genus items - Total:', genusOptions.length, 'Filtered:', filtered.length);
                          return filtered;
                        })().map((label, idx) => {
...
const styles = StyleSheet.create({
  ...
  appliesCard: { backgroundColor: '#FFFFFF', minHeight: 48, borderRadius: 12, paddingHorizontal: 12, paddingVertical: 8, alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between' },
  ...
  genusActions: { flexDirection: 'row', paddingTop: 12, gap: 12, backgroundColor: '#FFFFFF', paddingHorizontal: 24 },
